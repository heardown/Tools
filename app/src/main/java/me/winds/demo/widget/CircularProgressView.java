...
public class CircularProgressView extends View {
    ...
    float[] positions = {0.08f, 0.25f, 0.48f, 0.64f, 0.95f, 1f};
//    float[] positions = null;
//    float[] positions = {0.10f, 0.3f, 0.45f, 0.9f, 1.0f};
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //求最小值作为实际值,这是直径
        int minSize = Math.min(w - getPaddingLeft() - getPaddingRight() - outerArcWidth / 2,
                h - getPaddingTop() - getPaddingBottom() - outerArcWidth / 2);
        ...
        rotateMatrix.setRotate(startAngle, centerPoint.x, centerPoint.y);
...
//        indicatorCirclePaint.setMaskFilter(new BlurMaskFilter(indicatorCircleRadius / 2, BlurMaskFilter.Blur.NORMAL));
        ...
        pickerGradient = new ColorPickerGradient(gradientColors, positions); //文本选色
    ...
    private void drawText(Canvas canvas) {
        ...
        int color = pickerGradient.getColor(transformingValue / 100f * 0.67f);
        textPaint.setFakeBoldText(true);
        textPaint.setColor(color);
        int numSize = sp2px(getContext(), 32);
        textPaint.setTextSize(numSize);
        int width = calcTextWidth(textPaint, String.valueOf(transformingValue));
        int numY = centerPoint.y + numSize / 2 - (dp2px(getContext(), isEmpty ? 3 : 10));
    ...
    public void startAnimator() {
        animator = ValueAnimator.ofInt(0, currentValue);
     ...
     * @return
     */
    public CircularProgressView setAnimatorDuration(int duration) {
    ...
    private void needToReset(boolean needAnim) {
        if (needAnim) {
            tempValue = 0;
            startAnimator();
        } else {
            tempValue = currentValue;
            transformingValue = currentValue;
            invalidate();
        }
    }
    ...
    public void setCurrentValue(int currentValue, String diffValue) {
        setCurrentValue(currentValue, diffValue, true);
    }

    public void setCurrentValue(int currentValue, String diffValue, boolean needAnim) {
